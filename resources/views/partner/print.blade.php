...
<table width="100%">
    ...
    <tr>
        <td>Tanggal</td>
        <td align="right">{{ \Carbon\Carbon::parse($invoice['created_at'])->format("d/M/Y") }}</td>
    </tr>
    <tr>
        <td>Toko Pembeli</td>
        <td align="right">{{ $partner['nama'] }}</td>
    </tr>
</table>
<hr>
<table width="100%">
    @foreach($invoice['detail'] as $detail)
        <?php $total += ($detail['qty'] * $detail['harga']); $status = $detail['status']; ?>
        <tr>
            <td>{{ $detail['nama'] }}</td>
...
<table style="text-align:center;width:100%">
    ...
    <tr>
        <td><?php echo strtoupper($status == 1 ? "belum lunas" : "lunas") ?></td>
    </tr>
    <tr>
        <td>{{ $date }}</td>
    </tr>
